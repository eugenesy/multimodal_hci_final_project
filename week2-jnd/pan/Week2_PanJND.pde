...
float difference = 0.8;    // Max panning difference (0 = center, 1.0 = full left/right)
float step = 0.2;          // Pan difference step
...
int timerStart = 0;
int delayDuration = 0;
int toneDurationRef = 300; // 300ms each
...
void draw(){
  ...
  if (currentState == STATE_IDLE) {
    ...
    textSize(28);
    text("Stereo Panning (Spatial) JND Study", width/2, height/2 - 100);
    
    textSize(18);
    text("Instructions: Put on headphones!", width/2, height/2 - 40);
    text("1. You will hear TWO short sounds.", width/2, height/2 - 10);
    text("2. Did the second sound play from a different POSITION (left/right)?", width/2, height/2 + 20);
  ...
  // 2. END SCREEN (THE REPORT)
    ...
    textSize(18);
    text("Parameter Tested: Stereo Panning (Spatial Location)", 40, 90);
    ...
    textSize(22);
    fill(200, 0, 0); 
    text("JND Tracking Threshold: " + nf(final_threshold, 1, 3), 40, 260);
  ...
  // 3. ONGOING TEST SCREEN
    ...
    // textAlign(LEFT, TOP);
    // textSize(12);
    // text("Debug Pan Diff: " + nf(difference, 1, 4), 10, 10);
    ...
    if (currentState == STATE_DELAY) {
      ...
      textAlign(CENTER, CENTER);
      textSize(24);
      text("Did the position change?", width/2, height/2 + 20);
      textSize(18);
...
void playComparison() {
  // Randomly pan left (-difference) or right (+difference)
  float panVal = difference;
  if (random(1) > 0.5) panVal = -difference;
...
void drawGraph() {
  ...
  text(nf(yMax, 1, 2), graphX - 10, graphY);
  text(nf(yMin, 1, 2), graphX - 10, graphY + graphH);
  
  pushMatrix();
  translate(graphX - 55, graphY + graphH/2);
  rotate(-HALF_PI);
  textAlign(CENTER, CENTER);
  text("Stereo Panning Ratio", 0, 0);
  popMatrix();
}
